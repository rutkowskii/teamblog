...
namespace TeamBlog.Tests
{
    public class PostsTest : TestBase
    {
        ...
        public void AddThenRetrievePosts()
        {
            ...
            {
                actualPosts = K.Resolve<GetLatestChannelPostsQueryBuilder>().Build(ChannelId).Run();
            });
        ...
        private Guid ChannelId
        {
            get
            {
                return this.K.MongoAdapter.ChannelCollection.AsQueryable()
        ...
        private void InsertPost(string url, string content)
        {
            K.Resolve<InsertPostCommandBuilder>()
                .Build(ChannelId, url, content, Guid.Empty) //todo user id. 
                .Run();
        }
    }
}